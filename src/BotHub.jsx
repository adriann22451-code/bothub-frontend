...
const categories = [
  { label: "Spot Bots", icon: "◎", color: "#2DE0A6" },
  { label: "Futures Bots", icon: "▲", color: "#FF8A5C" },
  { label: "DEX Bots", icon: "◈", color: "#5CA8FF" },
  { label: "Arbitrage", icon: "⇄", color: "#FFB454" },
  { label: "AI Bots", icon: "✦", color: "#B98BFF" },
  { label: "Grid Bots", icon: "▦", color: "#5CA8FF" },
  { label: "Meme Bots", icon: "◕", color: "#FFD35C" },
  { label: "All Bots", icon: "▤", color: "#8A8AA3" },
];
...
function BottomNav({ active, setActive }) {
  const items = [
    ...
    { key: "market", label: "Marketplace", icon: LayoutGrid },
...
function HomeScreen({ goMarket, openBot }) {
  ...
  return (
    <div className="flex-1 overflow-y-auto px-5 pb-6 relative">
      ...
      <button
        onClick={goMarket}
        className="w-full text-left rounded-3xl p-5 relative overflow-hidden mb-6"
        style={{ background: "linear-gradient(135deg,#6C4FFF 0%,#4B2FD9 100%)" }}
      >
        ...
        <p className="text-[19px] font-bold text-white leading-snug max-w-[65%]">
          Trade Smarter With Powerful Trading Bots
        </p>
        <span className="inline-flex items-center gap-1 mt-4 px-4 py-2 rounded-xl text-[13px] font-semibold text-[#4B2FD9]" style={{ background: "#fff" }}>
          Explore Bots <ArrowUpRight size={14} />
        </span>
      </button>

      <div className="flex items-center justify-between mb-3">
        <h2 className="text-[15px] font-semibold text-[#F1F0F7]">Categories</h2>
        <button onClick={goMarket} className="text-[12px] font-medium" style={{ color: "#7B5CFF" }}>
          View all
        </button>
      </div>
      <div className="grid grid-cols-4 gap-3 mb-6">
        {categories.map((c) => (
          <button key={c.label} onClick={goMarket} className="flex flex-col items-center gap-2 py-3 rounded-2xl" style={{ background: "#C68B59", border: "1px solid #A9714B" }}>
            <span className="w-9 h-9 rounded-xl flex items-center justify-center text-[15px]" style={{ background: "rgba(255,255,255,0.35)", color: "#3B2A1E" }}>
              {c.icon}
            </span>
            <span className="text-[10px] text-[#3B2A1E] font-medium text-center leading-tight px-1">{c.label}</span>
          </button>
        ))}
      </div>
...
function MarketplaceScreen({ openBot }) {
  const [filter, setFilter] = useState("All");
  const [query, setQuery] = useState("");
  const filters = ["All", "Spot", "Futures", "DEX", "AI", "Arbitrage"];
  const filtered = bots.filter((b) => {
    const matchesFilter = filter === "All" || b.tag.toLowerCase() === filter.toLowerCase();
    const matchesQuery = b.name.toLowerCase().includes(query.toLowerCase());
    return matchesFilter && matchesQuery;
  });
  return (
    <div className="flex-1 overflow-y-auto px-5 pb-6">
      <h1 className="text-[19px] font-bold text-[#F1F0F7] pt-4 pb-4">Marketplace</h1>
      <div className="flex items-center gap-2 px-3.5 py-2.5 rounded-xl mb-4" style={{ background: "#C68B59", border: "1px solid #A9714B" }}>
        <Search size={16} color="#63637C" />
        <input
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          placeholder="Search bots..."
          className="bg-transparent outline-none text-[13px] text-[#F1F0F7] placeholder-[#63637C] w-full"
        />
      </div>
      ...
      <div className="flex flex-col gap-3">
        ...
        {filtered.length === 0 && (
          <p className="text-center text-[13px] text-[#63637C] py-10">No bots match that search.</p>
        )}
      </div>
    </div>
  );
...
export default function BotHubApp() {
  ...
  if (view?.type === "botDetail") {
    ...
    content = <HomeScreen goMarket={goMarket} openBot={openBot} />;
  } else if (tab === "market") {
    content = <MarketplaceScreen openBot={openBot} />;
  } else if (tab === "mybots") {
    content = (
      <MyBotsScreen
